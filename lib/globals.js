 ...
 */
global.scope = (name, fixtures, func) => {
    ...
    describe(name, U.wrap(fixtures, func));
};
/**
...
global.session = (name, ctx, func) => {
    ...
    var fixtures = [];
    if (CONF.useXvfb) fixtures.push(fixXvfb);
    if (CONF.isWeb && !CONF.webdriver.host) fixtures.push(fixSelenium);
    if (CONF.proxy.http) fixtures.push(fixHttpProxy);
    if (CONF.proxy.global) fixtures.push(fixGlobalProxy);
    if (CONF.isWeb) fixtures.push(fixWebdriver);
    if (CONF.isWeb) fixtures.push(fixBrowser);
...
global.beforeSession = ctx => {
    ...
    if (CONF.isWeb) before(async () => await _killProcs("chromedriver"));
};
/**
