...
config.uncaught = (argsConfig.uncaught || "log").toLowerCase();
expect([ "log", "fail", "mocha" ],
       "Invalid `--uncaught` value").include(config.uncaught);
config.rootConftest = argsConfig.rootConftest;
...
config.proxy.reconnect = argsConfig.reconnect || 1;
...
config.globalProxyPort = argsConfig.globalProxyPort || 8888;
