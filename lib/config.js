...
var argv = require("yargs")
    .usage("glace [options] [tests-file-or-folder]")
    .options({
        "web": {
            describe: "Flag to launch tests in browser. Browser will be " + 
                      "launched on session start and closed on session finish.",
            type: "boolean"
        },
        "app": {
            describe: "Application URL which will be used for web tests.",
            type: "string"
        },
        "dont-install-drivers": {
            describe: "Flag to not install selenium drivers on tests run.",
            type: "boolean"
        },
        "platform": {
            describe: "Specify platform type where tests will be executed.",
            type: "string",
            default: "pc",
            choices: [ "pc", "android", "ios" ]
        },
        "browser": {
            describe: "Name of browser where web tests will be " +
                      "executed. Default value is platform specific.",
            type: "string"
        },
        "retry": {
            describe: "Number of times to retry failed test.",
            type: "number",
            default: 0
        },
        "chunk-retry": {
            describe: "Number of times to retry failed chunk.",
            type: "number",
            default: 0
        }
    })
...
config.appUrl = argConfig.app;
config.installDrivers = !argConfig.dontInstallDrivers;  // default `true`
...
config.chunkRetries = argConfig.chunkRetry || 0;
...
config.useProxy = !!argConfig.proxy;  // default `false`
config.proxyPort = argConfig.proxyPort || 0;
config.useGlobalProxy = !!argConfig.globalProxy;  //default `false`
config.globalProxyPort = argConfig.globalProxyPort || 8080;
config.installCertificate = !!argConfig.installCertificate;  //default `false`
config.captureVideo = !!argConfig.video || !!argConfig.forceVideo;  // default `false`
config.forceVideo = !!argConfig.forceVideo;  // default `false`
config.cache.use = !!argConfig.cache || !!argConfig.existingCache;  // default `false`
config.cache.existing = !!argConfig.existingCache;  // default `false`
...
if (config.platform === "pc") {
    ...
    if (desired.browserName === "chrome") {
        ...
        if (config.useGlobalProxy) {
            desired.chromeOptions.args.push("ignore-certificate-errors");
            desired.chromeOptions.args.push(
                "proxy-server=" + os.hostname() + ":" + config.globalProxyPort);
            desired.chromeOptions.args.push(
                "proxy-bypass-list=localhost,127.0.0.1," + os.hostname());
        };
    };
} else {
