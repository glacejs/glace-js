...
module.exports = {
    ...
    makeScreenshot: async function (opts) {
        ...
        var imagePath = path.resolve(CONF.reportsDir,
        ...
        if (bySelenium) {
            ...
            await this._canvasScreenshot(imagePath);
        };
    ...
    /**
     * Step to check one image includes or doesn't inclue another image.
     *
     * @method
     * @protected
     * @instance
    ...
    _seleniumScreenshot: async function (imagePath) {
        await this._driver.saveScreenshot(imagePath);
    },
    /**
    ...
    _canvasScreenshot: async function (imagePath, opts) {
        ...
        if (cssSelector) errMsg += " of element with selector " + selector;

        await this._driver.execute(function (cssSelector) {
    ...
    _cutElement: async function (imagePath, elementName, opts) {
        ...
        var element = this.curPage[elementName];
        expect(element,
               `Undefined DOM element ${elementName}`).not.be.undefined;

        opts = U.defVal(opts, {});
        var targetName = U.defVal(opts.name, uuid()) + ".png";
        var targetPath = U.defVal(opts.path, U.mkpath(CONF.reportsDir,
                                                      CONF.testCase.name,
                                                      "screenshots",
                                                      "cutElements",
                                                      targetName));
