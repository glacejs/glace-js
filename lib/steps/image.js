...
module.exports = {
    ...
    makeScreenshot: async function (opts) {
        ...
        imageName = _.kebabCase(imageName);
    ...
    checkOrMakeScreenshot: async function (imageName, opts) {
        ...
        if (CONF.compareImages) {
            testName = CONF.curTestCase ? _.kebabCase(CONF.curTestCase.name) : "";
     ...
     * @arg {string} [opts.cutDirPath] - Folder path to cut elements.
     * @return {Promise<string>} - Path to cut element image.
    ...
    _cutElement: async function (imagePath, elementName, opts) {
        ...
        expect(element,
               `Undefined DOM element ${elementName}`).to.exist;
        ...
        targetPath = U.mkpath(
            U.defVal(
                opts.cutDirPath,
                path.resolve(getScreensDir(), "cutElements")),
            targetName);
