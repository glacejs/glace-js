...
module.exports = {
    ...
    removeVideo: function (opts) {
        ...
        if (!this._video.filePath || !fs.existsSync(this._video.filePath)) {
        ...
        fs.unlinkSync(this._video.filePath);

        if (check) {
            expect(fs.existsSync(this._video.filePath),
                   `Video file '${this._video.filePath}' wasn't removed`)
                .to.be.false;
        };
    },
};
