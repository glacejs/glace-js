...
module.exports = {
    ...
    /**
     * Step to stop selenium server. If selenium server wasn't started
     *
     * @method
     * @instance
     * @async
    ...
    stopSeleniumServer: async function () {
        ...
        if (!this._seleniumProc) {
            logger.stepDebug("Step to start selenium server wasn't passed");
            return;
        };

        await new Promise((resolve, reject) => {

            this._seleniumProc.on("end", (code, signal) => {
    ...
    launchBrowser: async function (opts) {
        ...
        if (check) {
            expect(await this._webdriver.session(),
                   "Browser wasn't launched").to.not.be.empty;
        };
    ...
    closeBrowser: async function (opts) {
        ...
        if (!this._isBrowserLaunched) {
            logger.stepDebug("Step to launch browser wasn't passed");
            return;
        };
        ...
        await this.pause(1000, "webdriver process will be stopped");

        if (opts.check) {
            expect(await this._webdriver.session(),
                   "Browser wasn't closed").to.be.empty;
        };
